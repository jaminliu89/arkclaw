---
name: wecom-doc
description: 企业微信文档管理技能。支持企业微信文档、表格（在线表格）、智能表格和智能文档（原名智能主页）的管理。具体能力：(1) 以 Markdown 格式获取文档/表格/智能表格的完整内容 (2) 新建文档或智能表格 (3) 用 Markdown 覆写文档内容 (4) 创建智能文档，将本地 Markdown 文件发布为智能文档 (5) 导出智能文档内容为 Markdown 文件。用户提及 `https://doc.weixin.qq.com/xxxx` 格式的URL链接时，需要使用此技能。
---
...
| `851003` | `no authority` | 无文档权限。在编辑场景，通常是因为机器人不允许编辑由企业成员创建的文档。在**智能表格写入场景**下，若该报错发生在 `smartsheet_add_records` / `smartsheet_update_records`，可引导用户走 Webhook 兜底方案，详见 [webhook-fallback.md](../wecom-smartsheet/references/webhook-fallback.md)；其他接口则按权限问题排查。注意 Webhook 只能更新通过 Webhook 写入的记录，人工创建的记录无法更新 |

---

## 文档

适用品类：文档（doc_type=3）、表格（在线表格）和智能表格（doc_type=10）
适用 URL：`/doc/*`、`/sheet/*`、`/smartsheet/*`

适用场景：
1. 以 Markdown 格式导出获取文档 / 表格 / 智能表格完整内容（异步轮询）
...
- 通过 URL 读取文档：使用 `wecom_mcp` tool 调用 `wecom_mcp call doc get_doc_content '{"url": "https://doc.weixin.qq.com/doc/xxx", "type": 2}'`
- 通过 URL 读取表格（在线表格）：使用 `wecom_mcp` tool 调用 `wecom_mcp call doc get_doc_content '{"url": "https://doc.weixin.qq.com/sheet/xxx", "type": 2}'`
...
> **关键提示**：读取内容前先看 URL 判断品类。`/doc/`、`/sheet/` 或 `/smartsheet/` → `get_doc_content`；`/smartpage/` → `smartpage_export_task`。只有用户明确提到「智能文档」或「智能主页」时才走 smartpage 流程，其他文档场景一律使用企微文档接口。

### 文档 / 表格 / 智能表格读取与文档编辑

1. **读取文档 / 表格 / 智能表格**（URL 含 `/doc/`、`/sheet/` 或 `/smartsheet/`） → 使用 `wecom_mcp` tool 调用 `wecom_mcp call doc get_doc_content '{"docid": "DOCID", "type": 2}'`，或通过 URL（`/doc/*`、`/sheet/*`、`/smartsheet/*` 均适用）：使用 `wecom_mcp` tool 调用 `wecom_mcp call doc get_doc_content '{"url": "https://doc.weixin.qq.com/sheet/xxx", "type": 2}'`，若 `task_done` 为 false 则携带 `task_id` 继续轮询
