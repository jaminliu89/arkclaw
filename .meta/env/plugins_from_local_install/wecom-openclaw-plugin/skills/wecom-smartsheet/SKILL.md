...
> ⚠️ 若返回 `errcode: 851003` 或 `errmsg` 包含 `no authority`，无文档权限。在编辑场景，通常是因为机器人不允许编辑由企业成员创建的文档。在**智能表格写入场景**下，若该报错发生在 `smartsheet_add_records` / `smartsheet_update_records`，可引导用户走 Webhook 兜底方案，详见 [webhook-fallback.md](references/webhook-fallback.md)。注意 Webhook 只能更新通过 Webhook 写入的记录，人工创建的记录无法更新。
...
4. **写入失败 fallback** → 第 2/3 步返回 `errcode: 851003` / `no authority`，无文档权限。在编辑场景，通常是因为机器人不允许编辑由企业成员创建的文档。在**智能表格写入场景**下，若该报错发生在 `smartsheet_add_records` / `smartsheet_update_records`，可引导用户走 Webhook 兜底方案，详见 [webhook-fallback.md](references/webhook-fallback.md)。注意 Webhook 只能更新通过 Webhook 写入的记录，人工创建的记录无法更新
